...
describe('TaskForm Component', () => {
  ...
  it('enables button when fields are filled', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Enter task title'), { target: { value: 'A' } })
    fireEvent.change(screen.getByPlaceholderText('Enter task description'), { target: { value: 'B' } })
  ...
  it('calls fetch and onTaskAdded on valid submit', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Enter task title'), { target: { value: 'Task 1' } })
    fireEvent.change(screen.getByPlaceholderText('Enter task description'), { target: { value: 'Desc 1' } })
  ...
  it('shows alert on failure', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText('Enter task description'), { target: { value: 'Desc' } })
    fireEvent.click(screen.getByText('Add Task'))

    await waitFor(() => {
      expect(global.alert).toHaveBeenCalledWith('Failed to add task. Please try again.')
    })
  })
})